...
app = FastAPI(title=f"{settings.APP_NAME} - Predictive Analytics", version="1.0.0", lifespan=lifespan, docs_url="/docs" if settings.DEBUG else None)
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "predictive-analytics", "infrastructure": get_infrastructure_status()}

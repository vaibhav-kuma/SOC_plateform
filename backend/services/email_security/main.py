...
app = FastAPI(title=f"{settings.APP_NAME} - Email Security", version="1.0.0", lifespan=lifespan, docs_url="/docs" if settings.DEBUG else None)
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "email-security", "infrastructure": get_infrastructure_status()}

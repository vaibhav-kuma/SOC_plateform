...
app = FastAPI(title=f"{settings.APP_NAME} - Autonomous SOC Orchestration", version="1.0.0", lifespan=lifespan, docs_url="/docs" if settings.DEBUG else None)
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "autonomous-soc", "infrastructure": get_infrastructure_status()}
